...
# I could do both solutions in a single line using a tuple or even just putting them into the print but these blanks are left to be filled by the reader.
part1 = [sum([1 if any([m <= id <= M for m, M in ranges]) else 0 for id in ids])for ranges, ids in[tuple(map(lambda block: [tuple(map(int, line.split("-"))) if "-" in line else int(line)for line in block.strip().split("\n")],open("input/day5.txt").read().split("\n\n"),))]][0]
part2=sum([M-m+1 for m,M in[[(r,r.__setitem__(i-1,(((min(r[i][0],r[i-1][0]),max(r[i][1],r[i-1][1]),),(0,-1))if((r[i][1]>=r[i-1][0]and r[i][1]<=r[i-1][1])or(r[i-1][0]>=r[i][0]and r[i-1][0]<=r[i][1])or(r[i-1][1]>=r[i][0]and r[i-1][1]<=r[i][1])or(r[i][0]>=r[i-1][0]and r[i][0]<=r[i-1][1]))else(r[i-1],r[i]))[0]),r.__setitem__(i,(((min(r[i][0],r[i-1][0]),max(r[i][1],r[i-1][1])),(0,-1))if((r[i][1]>=r[i-1][0]and r[i][1]<=r[i-1][1])or(r[i-1][0]>=r[i][0]and r[i-1][0]<=r[i][1])or(r[i-1][1]>=r[i][0]and r[i-1][1]<=r[i][1])or(r[i][0]>=r[i-1][0]and r[i][0]<=r[i-1][1]))else(r[i-1],r[i]))[1]))[0]for i in[i for i in range(len(r)-1,0,-1)]][0]for r in[sorted([r for r in tuple(map(lambda t:[(tuple(map(int,l.split("-")))if"-"in l else int(l))for l in t.strip().split("\n")],open("input/day5.txt").read().split("\n\n"),))[0]],key=lambda r:r[1],)]][0]])
